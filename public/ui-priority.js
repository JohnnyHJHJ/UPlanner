...
(function () {
  ...
  var birthdayFiltersExpanded = false;
  ...
  function loginHint() {
    ['li-input', 'su-input'].forEach(function (inputId) {
      var input = document.getElementById(inputId);
      var hintId = 'up-' + inputId + '-hint';
      if (!input || document.getElementById(hintId)) return;
      var hint = document.createElement('p'); hint.id = hintId; hint.className = 'up-login-hint hidden';
      hint.textContent = inputId === 'su-input' ? 'Another person is named Jed. This account will be created as jed2.' : 'Another person is named Jed. If this is you, sign in as jed2.';
      input.insertAdjacentElement('afterend', hint);
      var update = function () { hint.classList.toggle('hidden', !/^jed$/i.test(input.value.trim())); };
  ...
  function birthdayView() {
    ...
    root.innerHTML = '<section class="up-birthday-page"><div class="up-page-head"><div><p class="up-kicker">People calendar</p><h1>Birthdays</h1><p>Find birthdays with the same search and tag filters you use in People.</p></div><button id="up-birthday-back" class="btn-secondary">Back</button></div><div id="up-birthday-controls" class="card up-birthday-controls"><input id="up-birthday-search" class="input-field" placeholder="Search by name or username…"><button id="up-birthday-saved" class="btn-secondary text-xs">Saved people</button><button id="up-birthday-clear" class="btn-secondary text-xs">Clear filters</button><div id="up-birthday-months" class="up-birthday-months"></div></div><div id="up-birthday-filters" class="card up-birthday-filters"><button id="up-birthday-filter-toggle" type="button" class="up-filter-toggle" aria-expanded="false">Filters <span aria-hidden="true">+</span></button><div id="up-birthday-filter-body" class="hidden"></div></div><div id="up-birthday-body" class="up-loading">Loading birthdays…</div></section>';
    ...
    document.getElementById('up-birthday-filter-toggle').addEventListener('click', function () { birthdayFiltersExpanded = !birthdayFiltersExpanded; renderBirthdayFilters(); });
  ...
  function renderBirthdayFilters() { var root = document.getElementById('up-birthday-filter-body'); var toggle = document.getElementById('up-birthday-filter-toggle'); if (!root || !toggle) return; var schools = birthdayCatalog('school_tags'); var degrees = birthdayCatalog('degree_tags'); var count = birthdayState.school.length + birthdayState.degree.length; toggle.innerHTML = 'Filters' + (count ? ' (' + count + ')' : '') + ' <span aria-hidden="true">' + (birthdayFiltersExpanded ? '−' : '+') + '</span>'; toggle.setAttribute('aria-expanded', birthdayFiltersExpanded); root.classList.toggle('hidden', !birthdayFiltersExpanded); root.innerHTML = '<div><strong>School</strong><div class="up-birthday-chips">' + (schools.length ? schools.map(function (tag) { return birthdayFilterChip(tag, 'school'); }).join('') : '<span>No visible school tags available.</span>') + '</div></div><div><strong>Degree / course</strong><div class="up-birthday-chips">' + (degrees.length ? degrees.map(function (tag) { return birthdayFilterChip(tag, 'degree'); }).join('') : '<span>No visible degree tags available.</span>') + '</div></div>';
  ...
  function syncBirthdayNav() {
    var button = document.getElementById('up-calendar-nav'); if (!button) return;
    var active = !!document.getElementById('up-birthday-body');
